...
@section('body')
    <x-app-shell title="Ordenes de compra vigentes">
        <section class="panel finance-active-panel" data-section-export-ready="true">
            <div class="panel-header">
                <div class="finance-active-title-row">
                    <h2>OC Vigentes</h2>
                    <span class="finance-active-total">Pendiente por pagar: ${{ number_format((float) $orders->sum('total'), 2) }}</span>
                </div>
                <div class="toolbar finance-active-toolbar">
                    <a class="button ghost finance-active-export" href="{{ route('reports.download', 'finance-active-excel') }}">Exportar Excel</a>
                </div>
            </div>
                    ...
                    <tbody>
                        @forelse ($orders as $order)
                            ...
                            <tr data-excel-filter-row>
                                <td data-excel-filter-value="{{ $order->folio }}">
                                    <strong>
                                        <a href="{{ route('finance.orders.print', $order) }}" target="_blank">{{ $order->folio }}</a>
                                    </strong>
                                </td>
                                                                <td data-excel-filter-value="{{ ($order->created_on ?? $order->created_at)?->format('d/m/Y') ?? 'Sin fecha' }}" data-excel-filter-date="{{ ($order->created_on ?? $order->created_at)?->format('Y-m-d') }}">
                                    {{ ($order->created_on ?? $order->created_at)?->format('d/m/Y') ?? 'Sin fecha' }}
                                </td>
                                <td data-excel-filter-value="{{ $order->buyer->name }}">{{ $order->buyer->name }}</td>
                                <td data-excel-filter-value="{{ $order->company->name }}">{{ $order->company->name }}</td>
                                <td data-excel-filter-value="{{ $order->provider->business_name }}">
                                    {{ $order->provider->business_name }}
                                    <small class="fine-print">{{ $order->provider->business_line }}</small>
                                </td>
                                <td data-excel-filter-value="{{ $order->quote_original_name ?: 'Sin cotizacion' }}">
                                    @if ($order->quote_file_path)
                                        <a class="attachment-pill" href="{{ route('finance.orders.quote', $order) }}" target="_blank"><span>Soporte</span>{{ $order->quote_original_name }}</a>
                                    @else
                                        Sin cotizacion
                                    @endif
                                </td>
                                <td data-excel-filter-value="${{ number_format((float) $order->total, 2) }}">${{ number_format((float) $order->total, 2) }}</td>
                                <td data-excel-filter-value="{{ $financeStatusLabel }}">
                                    <details class="status-menu">
                                        <summary class="status {{ \App\Support\UiStatus::purchaseOrderClass($order->status, 'finance') }}">{{ $financeStatusLabel }}</summary>
                                        <div class="status-menu-panel">
                                            @if ($order->status === 'sent')
                                                <form class="inline-form" method="POST" action="{{ route('finance.orders.approve', $order) }}">
                                                    @csrf
                                                    @method('PATCH')
                                                    <button class="button primary small" type="submit">Aprobar</button>
                                                </form>
                                                <form class="inline-form" method="POST" action="{{ route('finance.orders.reject', $order) }}" onsubmit="return confirm('Rechazar {{ $order->folio }}?')">
                                                    @csrf
                                                    @method('PATCH')
                                            ...
                                            @elseif ($order->status === 'approved')
                                                <a class="button primary small" href="{{ route('finance.orders.payment', $order) }}">Pagar</a>
                                                <form class="inline-form" method="POST" action="{{ route('finance.orders.reject', $order) }}" onsubmit="return confirm('Rechazar {{ $order->folio }}?')">
                                                    @csrf
                                                    @method('PATCH')
                                                    <input type="hidden" name="reason" value="No cumple criterios de autorizacion">
                                ...
                                <td class="{{ $dueDateClass }}" data-excel-filter-value="{{ $order->due_date->format('d/m/Y') }}" data-excel-filter-date="{{ $order->due_date->format('Y-m-d') }}"><span class="due-date-pill">{{ $order->due_date->format('d/m/Y') }}</span></td>
                                <td data-excel-filter-value="{{ $order->warehouse ?: 'Sin almacen asignado' }}">{{ $order->warehouse ?: 'Sin almacen asignado' }}</td>
                            </tr>
                        @empty
                            <tr>
