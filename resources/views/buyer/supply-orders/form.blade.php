...
@section('body')
    ...
    <x-app-shell title="Nueva orden de suministro">
        <section class="panel supply-order-panel">
            ...
            <form class="stack supply-order-form" method="POST" action="{{ route('buyer.supply-orders.store') }}">
                @csrf
                <div class="grid-2">
                    <label>Empresa
                    ...
                    <label>Fecha requerida<input name="delivery_date" type="date" value="{{ old('delivery_date', now()->toDateString()) }}"></label>
                </div>
                        ...
                        <table class="supply-items-table">
                            ...
                            <tbody data-supply-items-body>
                                @foreach ($itemRows as $index => $row)
                                    ...
                                    @endphp
                                    <tr data-supply-row data-row-index="{{ $rowIndex }}">
                                        <td>
                                            <div class="supply-picker" data-supply-picker>
                                                <input type="hidden" name="items[{{ $rowIndex }}][warehouse_catalog_item_id]" value="{{ $selectedId ?: '' }}" data-supply-product-id>
                                                <button class="supply-picker-toggle" type="button" data-supply-picker-toggle>
                                                    {{ $selectedProduct['label'] ?? 'Selecciona...' }}
                                                </button>
                                        ...
                                        <td><span data-supply-unit>{{ $selectedProduct['unit'] ?? '—' }}</span></td>
                                        <td><span data-supply-price>{{ $selectedProduct['price'] ?? '—' }}</span></td>
                                        <td><input name="items[{{ $rowIndex }}][quantity]" type="number" min="0.01" step="0.01" value="{{ $row['quantity'] ?? '' }}" required></td>
                ...
                <div class="form-actions">
                    <a class="button ghost" href="{{ route('buyer.supply-orders.index') }}">Cancelar</a>
    ...
    <script type="application/json" id="supply-catalog-data">@json($productOptions)</script>
