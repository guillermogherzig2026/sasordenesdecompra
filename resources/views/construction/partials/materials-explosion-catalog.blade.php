...
>
    ...
    <div class="materials-category-list" data-materials-category-list>
        @foreach ($materialsCategories as $category)
            <section class="materials-category" data-materials-category="{{ $category['key'] }}">
                <button
                ...
                >
                    <span class="materials-square-toggle" data-materials-toggle-symbol aria-hidden="true">{{ $category['expanded'] ? '-' : '+' }}</span>
                    <strong>{{ $category['name'] }}</strong>
                    <span class="materials-count-badge">{{ count($category['concepts']) }} conceptos</span>
                    <span class="materials-chevron" data-materials-chevron aria-hidden="true">{{ $category['expanded'] ? '^' : 'v' }}</span>
                </button>

                <div class="materials-category-details" data-materials-category-details @if (! $category['expanded']) hidden @endif>
                    @if (! empty($category['note']))
                        <p class="materials-category-note">{{ $category['note'] }}</p>
                    @endif
                    ...
                    <div class="materials-concept-scroll">
                        <div class="materials-concept-list" data-materials-concepts>
                            @foreach ($category['concepts'] as $concept)
                                ...
                                @endphp
                                <section class="materials-concept" data-materials-concept="{{ $concept['key'] }}">
                                    <div class="materials-concept-summary">
                                        <button
                                            class="materials-square-toggle"
                                            type="button"
                                            data-materials-concept-toggle
                                            aria-label="{{ $concept['expanded'] ? 'Ocultar' : 'Mostrar' }} insumos de {{ $concept['name'] }}"
                                            aria-expanded="{{ $concept['expanded'] ? 'true' : 'false' }}"
                                        >{{ $concept['expanded'] ? '-' : '+' }}</button>
                                        <strong>{{ $concept['name'] }}</strong>
                                        <span>{{ number_format((float) $concept['quantity'], 2) }}</span>
                                        <span>{{ $concept['unit'] }}</span>
                                        <span class="materials-count-badge">{{ count($concept['supplies']) }} insumos</span>
                                    ...
                                    <div class="materials-concept-details" data-materials-concept-details @if (! $concept['expanded']) hidden @endif>
                                        <div class="table-scroll materials-supply-scroll">
                                            <table class="materials-supply-table" data-no-column-tools>
                                                <thead>
                                                    <tr>
                                                        <th>Insumo</th>
                                                        <th>Dosificacion por {{ $concept['unit'] }}</th>
                                                ...
                                                <tbody data-materials-supply-rows>
                                                    @foreach ($concept['supplies'] as $item)
                                                        @php
                                                            $required = (float) $item['dosage'] * (float) $concept['quantity'] * (1 + ((float) $item['waste'] / 100));
                                                            $amount = $required * (float) $item['unit_cost'];
                                                        @endphp
                                                        <tr data-materials-supply-row>
                                                            <td><input type="text" value="{{ $item['name'] }}" data-materials-supply-name aria-label="Insumo" readonly></td>
                                                            <td><input type="number" min="0" step="0.01" value="{{ number_format((float) $item['dosage'], 2, '.', '') }}" data-materials-dosage aria-label="Dosificacion" readonly></td>
                                                            <td>{{ $item['dosage_unit'] }}</td>
                                                            <td>
                                                                <span class="materials-inline-value">
                                                                    <input type="number" min="0" step="0.01" value="{{ number_format((float) $concept['quantity'], 2, '.', '') }}" data-materials-concept-quantity aria-label="Cantidad del concepto" readonly>
                                                                    <span>{{ $concept['unit'] }}</span>
                                                                </span>
                                                            </td>
                                                            <td>
                                                                <span class="materials-inline-value">
                                                                    <input type="number" min="0" step="0.01" value="{{ number_format((float) $item['waste'], 2, '.', '') }}" data-materials-waste aria-label="Merma" readonly>
                                                                    <span>%</span>
                                                                </span>
                                                            </td>
                                                            <td><strong data-materials-required>{{ number_format($required, 2) }}</strong></td>
                                                            <td>{{ $item['purchase_unit'] }}</td>
                                                            <td>
                                                                <span class="materials-currency-input">
                                                                    <span>$</span>
                                                                    <input type="number" min="0" step="0.01" value="{{ number_format((float) $item['unit_cost'], 2, '.', '') }}" data-materials-unit-cost aria-label="Costo unitario" readonly>
                                                                </span>
                                                            </td>
                                                            <td><strong data-materials-amount>${{ number_format($amount, 2) }}</strong></td>
                                                            <td>
                                        ...
                                        <div class="materials-concept-footer">
                                            <strong>Base de calculo: {{ number_format((float) $concept['quantity'], 2) }} {{ $concept['unit'] }} por concepto</strong>
                                            <strong data-materials-concept-total>${{ number_format($conceptTotal, 2) }}</strong>
                                        </div>
                                    </div>
                                </section>
                            @endforeach
